        ...
        if response.status_code == 201:
            ...
            if response.status_code == 200:
                ...
                response = requests.get(
                    f'{BASE_URL}/api/v1/chat/rooms/', headers=headers)
                print(
                    f"GET /api/v1/chat/rooms/ - Status: {response.status_code}")
                ...
                response = requests.post(
                    f'{BASE_URL}/api/v1/chat/rooms/', json=room_data, headers=headers)
                print(
                    f"POST /api/v1/chat/rooms/ - Status: {response.status_code}")
                ...
                if response.status_code == 201:
                    ...
                    # Test JOIN endpoint
                    response = requests.post(
                        f'{BASE_URL}/api/v1/chat/rooms/{room_id}/join/', headers=headers)
                    print(
                        f"POST /api/v1/chat/rooms/{room_id}/join/ - Status: {response.status_code}")
                    ...
                    response = requests.get(
                        f'{BASE_URL}/api/v1/chat/rooms/{room_id}/messages/', headers=headers)
                    print(
                        f"GET /api/v1/chat/rooms/{room_id}/messages/ - Status: {response.status_code}")
